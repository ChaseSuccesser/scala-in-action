...
object PrintType {
  ...
  // type parameters:
//  def printStructure[A]: Unit = macro PrintTypeMacros.printStructureMacro[A]
...
//  def printSymbol[A]: Unit = macro PrintTypeMacros.printTypeSymbolMacro[A]
...
//  def printSymbol(value: Any): Unit = macro PrintTypeMacros.printTermSymbolMacro
  ...
  // This only prints meaningful output if we can inspect
  // `A` to get at its definition:
//  def printDecls[A]: Unit = macro PrintTypeMacros.printDeclsMacro[A]
}
